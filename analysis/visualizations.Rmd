...
ggplot()+
  ...
  #geom_pointrange(data = time_plot_1, aes(x = time_point, y = mean, ymin = ci_lower, ymax = ci_upper,col = group), alpha = .6, size = 0.75, size = 0.2, fatten = 0.3, position = position_dodge(width = 0.5))+
...
```{r}
ggsave("../writing/paper/figures/performance.pdf", width = 180, height = 80, units = "mm", scale = 1.75, bg = "white")
```
...
```{r}
lsc <- readModels("../writing/supplement/saves/test_apes_data_LS_cau.out")
lsi <- readModels("../writing/supplement/saves/test_apes_data_LS_inf.out")
lsq <- readModels("../writing/supplement/saves/test_apes_data_LS_quant.out")
lsg <- readModels("../writing/supplement/saves/test_apes_data_LS_gaze.out")

lsc2 <- readModels("../writing/supplement/saves/test_apes_data_LS_cau_2.out")
lsi2 <- readModels("../writing/supplement/saves/test_apes_data_LS_inf_2.out")
lsq2 <- readModels("../writing/supplement/saves/test_apes_data_LS_quant_2.out")
lsg2 <- readModels("../writing/supplement/saves/test_apes_data_LS_gaze_2.out")
lsd2 <- readModels("../writing/supplement/saves/test_apes_data_LS_delay_2.out")
```
...
bind_rows(  
bind_rows(
  lsc$parameters$unstandardized%>%mutate(task = "causality"),
...
bind_rows(
  lsc2$parameters$unstandardized%>%mutate(task = "causality"),
...
```{r}
lsm <- ggplot(lsep, aes(x = time_point, y = est, col = phase, pch = sig))+
  geom_hline(aes(yintercept = ref), lty = 3, alpha = .75)+
  labs(x = "Time point",y = "Latent state mean")+
  geom_line(aes(group = phase, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  #geom_line(data = lsep%>%filter(time_point > 14), aes(group = task, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  geom_pointrange(aes(ymin = lower_2.5ci, ymax = upper_2.5ci),position = position_dodge(width = 0.2), alpha = .75, fatten = 3)+
  scale_color_colorblind(name = "",labels = c("Phase 1", "Phase 2"), guide = "none")+
  facet_grid(~task)+
  scale_shape_manual(values = c(16,4), name = "", labels = c("includes 0", "different from 0"))+
  #geom_vline(xintercept = 14.5, size = 1)+
  scale_x_continuous(breaks = c(1,14), labels = c(1,14))+
  theme_few()+
  theme(legend.direction = "vertical", legend.position = c(0.5, 0.3), legend.box = "horizontal", legend.background = element_blank(), strip.background = element_blank(),
...
bind_rows(  
bind_rows(
  lsc$parameters$r2%>%mutate(task = "causality"),
...
slice(which(row_number() %% 2 == 1))%>%
  ...
  filter(!(task == "gaze_following" & time_point == 8),
         !(task == "causality" & time_point == 7)),
bind_rows(
  lsc2$parameters$r2%>%mutate(task = "causality"),
...
```{r}
lsrel <- ggplot(lsrp, aes(x = time_point, y = est, col = phase))+
  labs(x = "Time point",y = "Reliability")+
  geom_line(aes(group = phase, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  #geom_line(data = lsep%>%filter(time_point > 14), aes(group = task, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  geom_pointrange(aes(ymin = lower_2.5ci, ymax = upper_2.5ci),position = position_dodge(width = 0.2), alpha = .75, fatten = 2)+
  scale_color_colorblind(name = "",labels = c("Phase 1", "Phase 2"))+
  facet_grid(~task)+
  #scale_shape_manual(values = c(16,4), name = "", labels = c("includes 0", "different from 0"))+
  #geom_vline(xintercept = 14.5, size = 1)+
  scale_x_continuous(breaks = c(1,14), labels = c(1,14))+
  ...
  theme_few()+
  theme(legend.direction = "vertical", legend.position = c(0.5, 0.3), legend.box = "horizontal", legend.background = element_blank(), strip.placement = "outside", strip.text = element_blank())
```


```{r}

plot_grid(lsm, lsrel, nrow = 2, labels = c("A", "B"), align = "v", rel_heights = c(1,1))

ggsave("../writing/paper/figures/latent_states_reliability.pdf", width = 180, height = 80, units = "mm", scale = 1.75, bg = "white")
```

```{r}
ggplot(lsrp, aes(x = time_point, y = est, col = task))+
  labs(x = "Time point",y = "Reliability estimate")+
  geom_line(aes(group = task, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  #geom_line(data = lsep%>%filter(time_point > 14), aes(group = task, x = time_point),position = position_dodge(width = 0.2), alpha = .75)+
  geom_pointrange(aes(ymin = lower_2.5ci, ymax = upper_2.5ci),position = position_dodge(width = 0.2), alpha = .75, fatten = 2)+
  scale_color_hue(name = "")+
  facet_grid(phase~., labeller=as_labeller(c("1" = "Phase 1", "2" = "Phase 2")))+
  #scale_shape_manual(values = c(16,4), name = "", labels = c("includes 0", "different from 0"))+
  #geom_vline(xintercept = 14.5, size = 1)+
  scale_x_continuous(breaks = c(1,14), labels = c(1,14))+
  scale_y_continuous(breaks = c(0,0.5,1), limits = c(0,1))+
  theme_few()+
  theme(legend.direction = "horizontal", legend.position = c(0.5, 0.47), legend.box = "horizontal", legend.background = element_blank(), strip.placement = "outside")
```
...
cor_data%>%
  ...
  ggplot(aes(x = time_span, y = value, col = phase))+
  geom_ysidedensity(aes(y = value, fill = phase),col = "white",alpha = 0.5)+
  #geom_xsidesegment(data =  cor_data_sum, aes(x = .lower, xend = .upper, y = 0, yend = 0, col = phase),alpha = 0.5)+
  #geom_xsidepoint(data =  cor_data_sum, aes(x = value, y = 0, col = phase),alpha = 0.75)+
  ...
  #xlim(-0.1,1)+
  labs(y = "Re-test correlation",
     x = "Distance between time points") +
  ...
  #ggside(collapse = "y", y.pos = "right", scales = "free_y")
  theme(#panel.border = element_rect(color = "black",fill = NA,size = .25),
...
```{r}
ggsave("../writing/paper/figures/reliability2.pdf", width = 180, height = 40, units = "mm", scale = 1.75, bg = "white")
```
...
# latent state trait models
lstc2 <- readModels("../writing/supplement/saves/test_apes_data_LST_cau_equSvar_2.out")
lsti2 <- readModels("../writing/supplement/saves/test_apes_data_LST_inf_equSvar_2.out")
lstq2 <- readModels("../writing/supplement/saves/test_apes_data_LST_quant_equSvar_2.out")
lstg2 <- readModels("../writing/supplement/saves/test_apes_data_LST_gaze_equSvar_2.out")
lstd2 <- readModels("../writing/supplement/saves/test_apes_data_LST_delay_equSvar_2.out")
lsti2p2 <- readModels("../writing/supplement/saves/test_apes_data_LST_inf_equSvar_2phases_2.out")
...
ggplot(lst,aes(x = param, y = est, col = phase, pch = part))+
  ...
  theme(#legend.text=element_text(size=7), legend.key.size = unit(0.4,"cm"),
    legend.position = c(0.95,0.25), legend.box = "vertical", legend.spacing.x = unit(0, 'cm'), legend.spacing.y = unit(0, 'cm'))
```

```{r}
ggsave("../writing/paper/figures/lstm.png", width = 10, height = 2, scale = 1.25, bg = "white")
```
...
```{r}
ggarrange(plstmpar, pmtm, nrow = 1, labels = c("A", "B"))
```
...
```{r}
ggsave("../writing/paper/figures/structure.pdf", width = 180, height = 50, unit = "mm", scale = 2, bg = "white")
```
...
grouproj <- proj_samples%>%
  ...
  facet_wrap_custom(task~., scales = "free_x", nrow = 1, scale_overrides = list(
    ...
    scale_override(3, scale_x_continuous(limits = c(-5, 5))),
...
ggsave(plot=xp,"../writing/paper/figures/ppi3_i6.pdf", width = 10, height = 6, scale = 1.25, bg = "white")
```
...
ggsave(plot=xp2,"../writing/paper/figures/ppi3_i7.pdf", width = 180, height = 100, unit = "mm", scale = 2, bg = "white")
```
