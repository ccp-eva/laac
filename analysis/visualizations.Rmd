...
p2f <- proj_samples_sum%>%
  ...
  mutate(factor= ifelse(pred == "Time spent in research","",factor),
         ...
         factor= factor(factor, levels = c("Group", "","Rearing \nhistory", "Observer \npresent"," ","  ","Sex","    ")))%>%
  ungroup()%>%
  mutate(pred2 = recode(pred,
                        Bonobo = 1,
                        `Chimpanzee group A` = 2,
                        `Chimpanzee group B` = 3,
                        Gorilla = 4,
                        Orangutan = 5,
                        `Time spent in research` = 6,
                        Age = 7,
                        `Hand-reared` = 8,
                        `Mother-reared` = 9,
                       no = 10, 
                        yes = 11,
                        `Sickness severity` = 12,
                        `Time spent outdoors` = 13,
                        Female = 14,
                        Male = 15,
                        Sociality = 16, 
                        Rank = 17,
                        `Study participation (day)` = 18))
...
p2 <- ggplot(p2f,aes(x = pred2, y = mean, col = phase)) +
  ...
  facet_grid(task~factor, scales = "free", space = "free_x", switch = "x")+
  labs(y = "Model estimate", x = "")+
  theme_minimal()+
  scale_color_colorblind(name = "")+
  scale_x_continuous(breaks = c(6,7,12,13,16,17,18), labels = c("Time spent in research","                           Age", "       Sickness severity","     Time spent outdoors", "               Sociality","                   Rank","Study participation (day)"),
                     sec.axis = sec_axis(~ .,breaks = c(1,2,3,4,5,8,9,10,11,14,15), labels = c("Bonobo","Chimpanzee group A","Chimpanzee group B", "Gorilla","Orangutan", "Hand-reared", "no", "yes", "Mother-reared", "Female","Male")))+
  theme(axis.text.x.bottom = element_text(angle = 45, hjust = 1, vjust = 1.4),
...
ptp <- proj_samples_sum%>%
  filter(pred == "Study participation (day)",
         task == "Quantity discrimination")%>%
  ggplot(aes(x = pred, y = mean, col = phase)) +
  geom_hline(yintercept = 0, lty = 3, alpha = .75)+
  geom_pointrange(aes(ymin = lci, ymax = uci),size = 0.25,pch = 4, position = position_dodge(width = 1))+
...
p2_i <- ggdraw() +
  draw_plot(p2) +
  draw_plot(pai, x = 0.39, y = .51, width = .04, height = .075)+
  draw_plot(pag, x = 0.39, y = .76, width = .04, height = .075)+
  draw_plot(ptq, x = 0.34, y = .387, width = .04, height = .075)+
  draw_plot(pti, x = 0.34, y = .51, width = .04, height = .075)+
  draw_plot(ptoc, x = 0.665, y = .635, width = .04, height = .075)+
  draw_plot(ptoq, x = 0.665, y = .387, width = .04, height = .075)+
  draw_plot(ptp, x = 0.895, y = .387, width = .04, height = .075)+
  draw_plot(ptd, x = 0.34, y = .26, width = .04, height = .075)

xp <- ggarrange(
ggarrange( p1, NULL , ncol = 1, heights = c(1,0.012)),
...
ggsave(plot=xp,"../writing/paper/figures/ppi3_i4.png", width = 10, height = 6, scale = 1.25, bg = "white")
```
