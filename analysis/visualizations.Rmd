...
p2f <- proj_samples_sum%>%
  ...
  mutate(factor= ifelse(pred == "Time spent in research","",factor),
         ...
         factor= factor(factor, levels = c("Group", "", "Observer \npresent"," ","Rearing \nhistory","  ","Sex","    ")))%>%
  ungroup()%>%
...
p2 <- ggplot(p2f,aes(x = pred2, y = mean, col = phase)) +
  ...
  theme_minimal()+
  scale_color_colorblind(name = "")+
  scale_x_continuous(breaks = c(6,9,12,15,16,17), labels = c("Time spent in research","                           Age", "       Sickness severity","                   Rank", "               Sociality","     Time spent outdoors"),
                     sec.axis = sec_axis(~ .,breaks = c(1,2,3,4,5,7,8,10,11,13,14), labels = c("Bonobo","Chimpanzee group A","Chimpanzee group B","Orangutan", "Gorilla", "no", "yes", "Hand-reared", "Mother-reared", "Female","Male")))+
  theme(axis.text.x.bottom = element_text(angle = 45, hjust = 1, vjust = 1.4),
                legend.direction = "horizontal",
        legend.position = c(0.14, 0.1), 
        legend.spacing.y = unit(0, 'cm'),
...
p2_i <- ggdraw() +
  draw_plot(p2) +
  draw_plot(pai, x = 0.509, y = .51, width = .03, height = .075)+
  draw_plot(pag, x = 0.509, y = .76, width = .03, height = .075)+
  draw_plot(ptq, x = 0.369, y = .387, width = .03, height = .075)+
  draw_plot(pti, x = 0.369, y = .51, width = .03, height = .075)+
  draw_plot(ptd, x = 0.369, y = .26, width = .03, height = .075)
...
```{r}
ggsave("../writing/paper/figures/ppi3_i3.png", width = 10, height = 6, scale = 1.25, bg = "white")
```
