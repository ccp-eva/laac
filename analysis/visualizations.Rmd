...
ggplot(lstar_beta,aes(x = task, y = est, col = phase, pch = phase))+
    geom_pointrange(aes(ymin = lower_2.5ci, ymax = upper_2.5ci), position = position_dodge(width = .7))+
  ...
  scale_shape(name = "")
  #theme(legend.position = "bottom", legend.box = "vertical")+
  theme(legend.text=element_text(size=7), legend.key.size = unit(0.4,"cm"),legend.position = c(0.4,0.7), legend.box = "vertical", legend.spacing.x = unit(0, 'cm'), legend.spacing.y = unit(0, 'cm'))
```
...
```{r}
ggarrange(plstarmpar, pmtm, widths = c(1,1), labels = c("A","B"))
```
...
p2 <- ggplot(proj_samples, aes(x = pred, y = mean, col = phase)) +
  geom_hline(yintercept = 0, lty = 3, alpha = .75)+
