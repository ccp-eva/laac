---
title: "Projection Prediction Inference: Covariate Selection"
...
- `varsel` is used as a quick inspection tool; `cv_varsel` for final selection
...
relevant_cov_vscau <- c(1, 2, 3, 4, 5, randint_ind_vscau)  # indices for relevant covariates
...
relevant_cov_vsquant <- c(1, 2, 3, 4, 5, 6, 7, randint_ind_vsquant)  # indices for relevant covariates
...
relevant_cov_vsgaze <- c(1, 2, 3, 4, 5, 6, 7, randint_ind_vsgaze)  # indices for relevant covariates
...
### final covariate selection (cross validated)
...
relevant_cov_cvscau <- c(1, 2, 3, 4, 5, randint_ind_cvscau)  # indices for relevant covariates

proj_cau_cv <- project(cvs_cau, solution_terms = relevant_cov_cvscau)
mcmc_areas(as.matrix(proj_cau_cv), 
           pars = solution_terms(cvs_cau)[relevant_cov_cvscau])
```
...
mcmc_areas(as.matrix(proj_inf_cv), 
           pars = solution_terms(cvs_inf)[relevant_cov_cvsinf])
```
...
relevant_cov_cvsquant <- c(1, 2, 3, 4, 5, 6, 7, randint_ind_cvsquant)  # indices for relevant covariates

proj_quant_cv <- project(cvs_quant, solution_terms = relevant_cov_cvsquant)
mcmc_areas(as.matrix(proj_quant_cv), 
           pars = solution_terms(cvs_quant)[relevant_cov_cvsquant])
```
...
relevant_cov_cvsgaze <- c(1, 2, 3, 4, 5, 6, 7, randint_ind_cvsgaze)  # indices for relevant covariates
