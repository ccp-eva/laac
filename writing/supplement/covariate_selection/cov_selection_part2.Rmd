...
t_cau <- filter(t_cau, time_point >= 15)
t_inf <- filter(t_inf, time_point >= 15)
t_quant <- filter(t_quant, time_point >= 15)
t_gaze <- filter(t_gaze, time_point >= 15)
t_grat <- filter(t_grat, time_point >= 15)
```
...
```{r}
#info <- debug_contr_error2(fm, t_cau)
#info
```
...
```{r, eval = F}
library(loo)
lapply(list(m_cau_2l, m_inf_2l, m_quant_2l, m_gaze_2l, m_grat_2l), loo)
```
...
refM_grat <- get_refmodel(m_grat_2l)
```
...
```{r, eval = F}
vs_grat <- varsel(refM_grat, search_terms = s_terms)
summary(vs_grat); plot(vs_grat, stats = c('elpd', 'rmse'))

randint_ind_vsgrat <- length(solution_terms(vs_grat))
relevant_cov_vsgrat <- c(1, 2, 3, 4, 5, randint_ind_vsgrat)
```
