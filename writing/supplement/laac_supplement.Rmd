...
pmtmcor <- mtmcor%>%
  ...
  scale_fill_gradient2(low = "#CC6677", high = "#117733", mid = "white", 
   ...
   name="Correlation")+
  facet_grid(~type)+
  geom_text(size = 3, fontface = ifelse(mtmcor$sig == T, 2, 1))+
  theme_few()+
...
proj_cau_cv <- project(cvs_cau,  solution_terms = c(1, 2, 15))

as.matrix(proj_cau_cv)%>%as_tibble()#%>%select(rel_rank, group)

mcmc_areas(as.matrix(proj_cau_cv))
...
```{r}
proj_cau_cv <- project(cvs_cau,  solution_terms = c(1, 9, 14))
...
ggplot(proj_samples_cau, aes(x = value, y = pred, fill = species)) +
...
ggplot(proj_samples_inf, aes(x = value, y = pred, fill = species)) +
  geom_vline(xintercept = 0, lty = 2, alpha = .75)+
  geom_density_ridges(alpha = .5, scale = .7) +
  facet_wrap(factor~., scales = "free")+
  scale_fill_colorblind()+
...
ggplot(proj_samples_gaze, aes(x = value, y = pred, fill = species)) +
  geom_vline(xintercept = 0, lty = 2, alpha = .75)+
  geom_density_ridges(alpha = .5, scale = .7) +
...
ggplot(proj_samples_quant, aes(x = value, y = pred, fill = species)) +
  geom_vline(xintercept = 0, lty = 2, alpha = .75)+
  geom_density_ridges(alpha = .5, scale = .7) +
  facet_wrap(factor~., scales = "free", nrow = 1)+
  scale_fill_colorblind()+
