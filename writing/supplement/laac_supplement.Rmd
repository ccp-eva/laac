...
We have two overarching questions. On the one hand, we are interested in the stability and the reliability of the individual task as as well as the relations between them. We use structural equation modeling to address these questions. These models have been developed and are usually used with much larger sample sizes. Thus, we had to make a number of assumptions to be able to fit them to the kind of data that we have, which we mention in the text below.

On the other hand, we 

Not analyse switching task

Two compoennts of the analysis and the different methods used for it
...
```{r lsgraph, engine='tikz', fig.cap="Latent State model for two indicators and six measurement time points."}
...
```{r lstgraph, engine='tikz', fig.cap="Latent State Trait model for two indicators and six measurement time points."}
...
```{r lstrgraph, engine='tikz', fig.cap="Latent State Trait model with autoregressive effects for two indicators and six measurement time points."}
