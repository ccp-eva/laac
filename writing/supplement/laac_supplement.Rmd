...
In sum, the different models suggest that group- and individual-level performance was relatively stable over time, with a notable exception in the earlier time points fo phase 2. As noted above -- and as can be seen in Figure \@ref(fig:perfplot) -- performance on a group level was clearly above chance.  
...
Taken together, quantity judgments were stable over time on the group level but less so on an individual level (see also Figure \@ref(fig:perfplot)).
...
```{r mtmplot, fig.height=4, fig.cap = "Phase 1: correlations between latent traits and latent state residual variables, respectively, of different tasks. Bold correlations are different from zero as judged by the 95\\% CI. The trait correlations between quantity and causality are not displayed, see main text for details. See main text for details.", out.width="100%"}
pmtmcor
```
...
The most notable difference between the two Phases was that we see a lot more substantial trait correlations in Phase 2 compared to Phase 1. The only significant trait correlation from Phase 1 also turned out to be significant in Phase 2 (quantity and inference by exclusion). These results suggest some commonalities between the tasks -- with gaze following being an exception (see Figure \@ref(fig:mtmplot2) and \@ref(fig:mtmplotc)). However, the sample size we studied here is still comparatively small for individual differences research looking for correlations between traits. Larger samples are needed to corroborate these findings. 

The significant *negative* state residual correlation between causal inference and inference found in Phase 1 was not present in Phase 2. This suggests a de-coupling between the two tasks in that apes were less likely to confuse the two tasks with one another.
