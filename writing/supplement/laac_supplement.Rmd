...
```{r}
# proj_inf_cv_p2 <- project(cvs_inf_p2,  solution_terms = c(1, 2, 3,4))
# 
...
The gaze following task was the only one for which some of the predictors selected in Phase 1 were not selected in Phase 2 (`rearing`, `time_outdoors`, `sociality`). However, except for `time_outdoors`, despite not being selected the ranking of these predictors was similar. The divergence for `time_outdoors` can largely be explained by the initial habituation we saw in gaze following at the beginning of Phase 1 (Figure \@ref(fig:perfplot)) which coincided with a seasonal decline in time spent outdoors (Figure \@ref(fig:glife)). At the beginning of Phase 2, gaze following rates remained low while time spent outdoors was high again. Thus, the result of Phase 1 is likely to be a mere coincident. Phase 2 had additional predictors selected (`observer`, `sex`) and overlapping predictors between phases were `group`, `age`, and `sex`.
