...
None of the latent state means -- except for time point 14 -- were significantly different from zero, suggesting a stable average level of gaze following in Phase 2 (Figure \@ref(fig:lseplot2)). Reliability was generally high, but variable, ranging between `r min(plsr2%>%filter(task == "gaze_following")%>%pull(est))` and `r max(plsr2%>%filter(task == "gaze_following")%>%pull(est))` [Manuel: hier ist irgendetwas schief gegangen. Hier steht +-unendlich. Es müsste between 0.567 and 0.885 heißen.]. The correlations between latent states for the different time points were generally high, pointing to stable individual differences (Figure \@ref(fig:lsplot2)).
...
Taken together, quantity judgments were stable over time on the group level but less so on an individual level [Manuel: die Interpretation ist mir nicht ganz klar. mit 94% Konsistenz sind inter-ind. Unterschiede doch ziemlich stabil? Die geringe Reliabilität sagt nichts über wahre individuelle Unterschiede. Oder verstehe ich den Satz falsch?]. Group-level performance was cleary different from chance (Figure \@ref(fig:perfplot)).
...
In sum, delay of gratification showed some variation in group level performance. However, differences between individuals were relatively stable over time (Figure \@ref(fig:perfplot)). [Manuel: ist ja weniger stabil als zB gaze following oder causal inference, mit einer konsistenz von .77]
...
The gaze following task showed notable differences between the two phases. The initial decline in gaze following in the beginning of Phase 1 did not repeat itself in Phase 2. Performance in Phase 2 seemed to pick up where it left off at the end of Phase 1 (Figure \@ref(fig:perfplot)). The LSM mean estimates for the different time points follow a similar pattern. Despite this difference in group-level variability [Manuel: meinst du average group-level performance ? weil variability spiegelt sich zB in der Konsistenz und diese ist ja in beiden Phasen ähnlich.], the near-identical LSTM estimates for consistency and occasion specificity showed that individual differences were structured in a similar way across the two phases (Figure \@ref(fig:lstdifplot)).
