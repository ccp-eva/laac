...
bind_rows(
  ...
  ylim(-0.25, 0.25)+
  theme_minimal()+
  labs(x = "", y = "Difference in model estiamte btw. phases")+
  scale_x_discrete(limits=rev)+
  theme(axis.text.x = element_blank(), legend.position = "bottom")+
  scale_color_colorblind(name = "Task")
...
```{r lstdifplot2, fig.height = 2.5, fig.cap = "Difference between model LSTM estimates for phase 1 and phase 2.", out.width="100%"}
plstdif
```
...
One (out of four) of the test-half specific trait correlations between causality and quantity was also reliably different from zero ($r_{tc2,tq1}$ = `r lstcq$parameters$stdyx.standardized%>%filter(paramHeader == "TC1.WITH", param == "TQ2")%>%pull(est)`, 95% CI = [`r lstcq$parameters$stdyx.standardized%>%filter(paramHeader == "TC1.WITH", param == "TQ2")%>%pull(lower_2.5ci)`; `r lstcq$parameters$stdyx.standardized%>%filter(paramHeader == "TC1.WITH", param == "TQ2")%>%pull(upper_2.5ci)`]). We do not consider this result to be substantial evidence for a substantive association between the trait estimates in the two tasks and therefore do not interpret it any further. Figure \@ref(fig:mtmplot) shows all correlations between the different tasks.
...
pmtmcorc <- mtmcorc%>%
  ...
  geom_point(aes(size = est, pch = phase), alpha = .75)+
  facet_grid(~type)+
    labs(x = "", y = "")+
  scale_fill_gradient2(low = "#CC6677", high = "#117733", mid = "#FFE066", 
   midpoint = 0, limit = c(-.5,.5), space = "Lab", 
   name="Difference \nbtw. correlations")+
  scale_shape_manual(values=c(0,1), name = "Phase")+
  guides(size = F)+
  theme_few()+
  scale_x_discrete(guide = guide_axis(n.dodge = 2),limits = c("causality", "quantity","gaze_following"))+
  scale_y_discrete(limits = c( "inference","gaze_following", "quantity"))
...
```{r mtmplotc, fig.height=4, fig.cap = "Differences between phases in correlations between latent traits and latent state residual variables for the different tasks. Shapes show correlations for each phase, with the size proportional to the correlation.", out.width="100%"}
pmtmcorc
```
