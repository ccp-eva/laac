...
A second coder unfamiliar to the purpose of the study coded 15% of all time points (four out of 28) for all tasks (except switching). Reliability was good to excellent. Gaze following: 92% agreement ($\kappa$ = .64), causal inference 99% agreement ($\kappa$ = .98), inference by exclusion: 99% agreement ($\kappa$ = .99), quantity discrimination: 99% agreement ($\kappa$ = .97), delay of gratification: 98% agreement ($\kappa$ = .97).
...
For the comparison, we will focus on the four tasks that were included in both phases: causal inference, inference by exclusion, gaze following and quantity.
...
The significant *negative* state residual correlation between causal inference and inference by exclusion found in Phase 1 was not present in Phase 2. This suggests a de-coupling between the two tasks in that apes were less likely to confuse them in Phase 2. This interpretation is also supported by the increase in group-level performance for inference by exclusion and an increase in the trait correlation between the two tasks.
...
```{r cselp, fig.height=4,fig.width=10, fig.cap = "Predictor selection for causal inference in Phase 1. A) Elpd and RMSE values for predictors, ordered by importance (left to right) according to the cross-validated projection prediction model. Note that the random intercept term was forced to be the last one to be included. B) Projections for the selected predictors based on the submodel.", out.width="100%"}
...
```{r cselp2, fig.height=4,fig.width=10, fig.cap = "Predictor selection for causal inference in Phase 2. A) Elpd and RMSE values for predictors, ordered by importance (left to right) according to the cross-validated projection prediction model. Note that the random intercept term was forced to be the last one to be included. B) Projections for the selected predictors based on the submodel.", out.width="100%"}
...
For causal inference, the highest ranked predictor was the same in both phases (`group`). However, in Phase 2, `time_outdoors` was also selected to be relevant even though it had received a fairly low rank in Phase 1.
