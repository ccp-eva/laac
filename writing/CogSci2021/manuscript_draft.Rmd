...
meta_plot <- ggplot(aes(b_Intercept, relevel(time_point, "Pooled Effect", after = Inf),fill = task), 
  ...
  geom_text(data = mutate_if(sd_task_summary, is.numeric, round, 2),
    aes(label = glue("{sd_time_point__Intercept} [{sd_time_point__Intercept.lower}, {sd_time_point__Intercept.upper}]"), x= Inf, y = time_point), hjust = "inward")+
  geom_pointintervalh(data = forest_data_task_summary%>%filter(time_point == "Pooled Effect"), size = 1)+
  geom_text(data = mutate_if(forest_data_task_summary%>%filter(time_point == "Pooled Effect"), is.numeric, round, 2), aes(label = glue("{b_Intercept} [{.lower}, {.upper}]"), x= Inf, y = time_point), hjust = "inward")+
...
```{r}
ggarrange(perf_plot,meta_plot, nrow = 2, ncol = 1)
```
...
ggplot(data = cor_data, aes(x = value , y = task ,fill = task)) +
  geom_vline(xintercept = 0, color = "black", size = .5, lty = 2) +
...
pred_plot <-  ggplot(data = pred, aes(x = value , y = task ,fill = task)) +
  ...
  #geom_segment(y = "Time point 1", yend = "Time point 1", x = -Inf, xend = Inf, color = "black", size = 0.5) +
  geom_vline(xintercept = 0, color = "black", size = .5, lty = 2) +
  geom_density_ridges(rel_min_height = 0.01,col = "black", scale = 1,
                      alpha = 0.5) +
