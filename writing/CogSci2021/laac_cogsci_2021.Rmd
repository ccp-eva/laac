...
author-information: > 
    \author{{\large \bf Manuel Bohn (manuel\_bohn@eva.mpg.de), Johanna Eckert (johanna\_eckert@eva.mpg.de), Daniel Hanus (hanus@eva.mpg.de) & (haun@eva.mpg.de) Daniel Haun} \\ Department of Comparative Cultural Psychology, Max Planck Institute for Evolutionary Anthropology, Deutscher Platz 6, 04103 Leipzig, Germany}
...
```{r 2-col-image3, fig.env = "figure*", fig.pos = "h", fig.width=7, fig.height=1.5, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "This image spans both columns. And the caption text is limited to 0.8 of the width of the document."}
p_meta
```
...
```{r 2-col-image2, fig.env = "figure*", fig.pos = "h", fig.width=7, fig.height=2, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "This image spans both columns. And the caption text is limited to 0.8 of the width of the document."}
...
We tested the same sample of great apes repeatedly on five cognitive tasks. This design allowed us to address some pressing questions in primate cognition research: How stable is group level performance in cognitive tasks? How reliable are the results of these taks? How much is performance influenced by individual characteristics? Below we discuss the results in light of these questions. 
...
Three out of five tasks showed acceptable levels of reliability. Importantly, reliability is independent from group level performance (leaving aside floor and ceiling effects) [see @hedge2018reliability]. Here, we see such a pattern for inference: Group level performance was consistently at chance level for every time point. On a group level, one would conclude that great apes did not make the inference in question. However, the tasks was highly reliable, suggesting that it accurately captured individual differences. Together with the observation that some individuals consistently performed at ceiling (see Figure XX), this suggests that the task is well suited to measure inferential abilities on a *individual* level. The opposite pattern holds for quantity. Here, group level performance was consistently above chance but individual differences were not very consistent. This suggests that variation was due to sources other than systematic differences between individuals. This phenomenon is quite common in the adult cognitive literature [hedge2018reliability] and arises when experimental tasks (optimized for low variance in measurement) are used to study individual differences (requiring high variance in measurement). Taken together, we may recommend that researchers select their tasks depending on the kind of questions they want to answer. When planning to study individual differences by relating measures to one another, researchers are well advised to first study the reliability of these measures. Even though this takes considerable time and effort, it increases to the chances of finding effects.  
