...
```{r 2-col-image, fig.env = "figure*", fig.pos = "h", fig.width=7, fig.height=2, fig.align = "center", set.cap.width=T, num.cols.cap=2, fig.cap = "This image spans both columns. And the caption text is limited to 0.8 of the width of the document."}
perf_plot
```
...
insets <- forest_data_task %>% 
  ...
  purrr::map(~annotation_custom2(
    ...
    xmin = 0.2, xmax = 1.8, ymin = 5, ymax = 15)
  )
...
meta_plot <- ggplot(aes(b_Intercept, fill = task), 
  ...
  geom_text(data = mutate_if(sd_task_summary, is.numeric, round, 2),
     aes(label = glue("{sd_time_point__Intercept} [{sd_time_point__Intercept.lower}, {sd_time_point__Intercept.upper}]"), x= Inf, y = 1), hjust = "inward", size = 4)+
...
Stability: Performance except for gaze following relatively stable. This is true even though subjects are differentially reinforced and recieve the exact same procedure and counterbalancing each time. This suggests that subjects are not very suceptible to these inforamtion sources, making it difficult to increase performance via associative learning. 
